...
class AllChatsViewController: UIViewController, TableViewFetchedResultsDisplayer, ChatCreationDelegate, ContextViewController {
    ...
    func deleteAll(){
        guard let chats = fetchedResultsController?.fetchedObjects as? [Chat]else {return}
        for chat in chats{
    ...
    func configureCell(cell:UITableViewCell, atIndexPath indexPath:NSIndexPath){
        ...
        guard let lastMessage = chat.lastMessage, timestamp = lastMessage.timestamp, text = lastMessage.text
        else {return}
